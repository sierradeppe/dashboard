...
begin
	rm("hetdex_sc1_detinfo_v3.2.ecsv"; force=true)
	rm("hetdex_sc1_v3.2.ecsv"; force=true)	
end
...
function download_hetdex_file(file_name)
    ...
    base_url = "http://web.corral.tacc.utexas.edu/hetdex/HETDEX/catalogs/hetdex_source_catalog_1/"
...
"""
Convert ECSV to DataFrame via CSV intermediate step
"""
function ecsv_to_dataframe(file_name)
    # Import needed Python modules
    astropy = pyimport("astropy.table")
    np = pyimport("numpy")
    
    # Read the ECSV file
    println("Reading file with astropy: $file_name")
    table = astropy.Table.read(file_name, format="ascii.ecsv")
    
    # Get column names
    columns = table.colnames
    println("Found $(length(columns)) columns")
    
    # Create a temporary CSV file
    csv_file = replace(file_name, ".ecsv" => ".csv")
    println("Converting to CSV: $csv_file")
    
    # Write to CSV (this avoids PyCall access issues)
    table.write(csv_file, format="csv", overwrite=true)
    
    # Read the CSV directly into a DataFrame
    println("Reading CSV into DataFrame")
...
"""
function process_hetdex_files()
    # List of files to process
    files = ["hetdex_sc1_detinfo_v3.2.ecsv", "hetdex_sc1_v3.2.ecsv"]
    
    # Download files if needed
    for file in files
        download_hetdex_file(file)
    end
...
function encode_source_type(df::DataFrame, source::String,og)
    ...
    mapping = Dict(source => 1, src[1] => 0, src[2] => 0)
	df_copy.source_type = [mapping[val] for val in df_copy.source_type]
...
function predict_source_type(row, fit_lae, fit_lzg, fit_agn)
    ...
    prob_lzg = predict(fit_lzg, row)[1]
    prob_agn = predict(fit_agn, row)[1]
    
    # Get the class with highest probability
    probs = [prob_lae, prob_lzg, prob_agn]
    classes = ["lae", "lzg", "agn"]
...
function predict_all_sources(df, fit_lae, fit_lzg, fit_agn)
    ...
    for i in 1:nrow(df)
        row = df[i:i, :]
        pred = predict_source_type(row, fit_lae, fit_lzg, fit_agn)
...
function evaluate_lr_model(df, predictions)
    ...
    for i in 1:length(actual)
        actual_idx = findfirst(classes .== actual[i])
        pred_idx = findfirst(classes .== predictions[i])
...
[[deps.AdvancedHMC]]
deps = ["AbstractMCMC", "ArgCheck", "DocStringExtensions", "LinearAlgebra", "LogDensityProblems", "LogDensityProblemsAD", "ProgressMeter", "Random", "Requires", "Setfield", "SimpleUnPack", "Statistics", "StatsBase", "StatsFuns"]
git-tree-sha1 = "3027ae3794d5205a5936272b02886a6c5ebaaa60"
...
git-tree-sha1 = "bcf97a5390262571d64d62566673bf1e72080d11"
...
git-tree-sha1 = "c017e6cded5495294ff82d5c8a176492f752b22e"
...
git-tree-sha1 = "6542cf0ecc30c506257e2c9544c65722eb8e5fb2"
...
[[deps.DynamicPPL]]
deps = ["ADTypes", "AbstractMCMC", "AbstractPPL", "Accessors", "BangBang", "Bijectors", "Compat", "ConstructionBase", "DifferentiationInterface", "Distributions", "DocStringExtensions", "InteractiveUtils", "LinearAlgebra", "LogDensityProblems", "MacroTools", "OrderedCollections", "Random", "Requires", "Test"]
git-tree-sha1 = "cf76563df6f94c6075b0dc6b1f4a9e565bf27681"
...
git-tree-sha1 = "f21402d4be463b3f5c4017e11fa00280fefc5c2f"
